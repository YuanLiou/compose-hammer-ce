...
abstract class BaseOperations<Input, T : Reorderable> {
    ...
    protected abstract fun copyWithNewOrder(
        item: T,
        newOrder: Double
    ): T
    ...
    fun addItem(input: Input): AddOperationResult<T> {
        val newItem =
            when (val maybeValid = createItem(input, items.findNextOrderNum(), null)) {
    ...
    fun editItem(
        ...
        val updatedItem =
            when (val maybeValid = createItem(input, item.order, item)) {
    ...
    fun moveItemUp(item: T): MoveOperationResult<T> {
        val newOrder =
            items.moveUp(item)
    ...
    private fun updateStateInternal(updatedItems: List<T>) {
        updateState(
    ...
    private fun <T : Reorderable> List<T>.findNextOrderNum(): Double = lastOrNull()?.order?.plus(2) ?: 1.0

    private fun <T : Reorderable> List<T>.ensureUniqueOrder(constructor: (item: T, order: Double) -> T) =
...
sealed interface AddOperationResult<out T> {
    data class AlreadyExists<T>(
        val item: T
    ) : AddOperationResult<T>
...
fun <T> AddOperationResult<T>.toResultEither(): Either<String, T> =
    when (this) {
